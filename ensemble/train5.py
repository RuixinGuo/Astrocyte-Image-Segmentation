...
class EnsembledModel(nn.Module):
    def __init__(self, model1, model2, model3, model4, model5):
        ...
        self.model4 = model4
        self.model5 = model5
        self.conv0 = nn.Conv2d(5, 1, kernel_size=(5, 5), stride=(1, 1), padding='same', bias=False)
        self.conv1 = nn.Conv2d(5, 1, kernel_size=(5, 5), stride=(1, 1), padding='same', bias=False)
    ...
    def forward(self, x):
        ...
        pred_0 = torch.cat((pred1[:, 0:1, :, :], pred2[:, 0:1, :, :], pred3[:, 0:1, :, :], pred4[:, 0:1, :, :], pred5[:, 0:1, :, :]), 1)
        pred_1 = torch.cat((pred1[:, 1:2, :, :], pred2[:, 1:2, :, :], pred3[:, 1:2, :, :], pred4[:, 1:2, :, :], pred5[:, 1:2, :, :]), 1)
...
model_ft = EnsembledModel(model_ft1, model_ft2, model_ft3, model_ft4, model_ft5)
