...
'''-----The Eighth Model-----'''
model_ft8 = smp.UnetPlusPlus(
        encoder_name="inceptionresnetv2", #"resnet34",          # choose encoder, e.g. mobilenet_v2 or efficientnet-b7
        encoder_weights="imagenet",             # use `imagenet` pre-trained weights for encoder initialization
        in_channels=1,                                  # model input channels (1 for gray-scale images, 3 for RGB, etc.)
        classes=2,                                              # model output channels (number of classes in your dataset)
        activation="softmax",
...
ct = 0
for name, param in model_ft8.named_parameters():
        ct += 1
        #print(str(ct) + ' ' + name) #Show the layers that contains parameters
        param.requires_grad = False  # Freeze these layers
...
ct = 0
for name, param in model_ft9.named_parameters():
        ct += 1
        #print(str(ct) + ' ' + name) #Show the layers that contains parameters
        param.requires_grad = False  # Freeze these layers
...
ct = 0
for name, param in model_ft10.named_parameters():
        ct += 1
        #print(str(ct) + ' ' + name) #Show the layers that contains parameters
        param.requires_grad = False  # Freeze these layers
...
class EnsembledModel(nn.Module):
    def __init__(self, model1, model2, model3, model4, model5, model6, model7, model8, model9, model10):
        ...
        self.model1 = model1.to(device=device0, dtype=torch.float32)
        self.model2 = model2.to(device=device0, dtype=torch.float32)
        self.model3 = model3.to(device=device0, dtype=torch.float32)
        self.model4 = model4.to(device=device0, dtype=torch.float32)
        self.model5 = model5.to(device=device0, dtype=torch.float32)
        self.model6 = model6.to(device=device1, dtype=torch.float32)
        self.model7 = model7.to(device=device1, dtype=torch.float32)
        self.model8 = model8.to(device=device1, dtype=torch.float32)
        self.model9 = model9.to(device=device1, dtype=torch.float32)
        self.model10 = model10.to(device=device1, dtype=torch.float32)
        self.conv0 = nn.Conv2d(10, 1, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=False)
        self.conv1 = nn.Conv2d(10, 1, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=False)
        self.softmax = nn.Softmax(dim = 1)

    def forward(self, x):
        x0 = x.to(device=device0, dtype=torch.float32)
        x1 = x.to(device=device1, dtype=torch.float32)
        pred1 = self.model1(x0).to(device="cpu", dtype=torch.float32)
        pred2 = self.model2(x0).to(device="cpu", dtype=torch.float32)
        pred3 = self.model3(x0).to(device="cpu", dtype=torch.float32)
        pred4 = self.model4(x0).to(device="cpu", dtype=torch.float32)
        pred5 = self.model5(x0).to(device="cpu", dtype=torch.float32)
        pred6 = self.model6(x1).to(device="cpu", dtype=torch.float32)
        pred7 = self.model7(x1).to(device="cpu", dtype=torch.float32)
        pred8 = self.model8(x1).to(device="cpu", dtype=torch.float32)
        pred9 = self.model9(x1).to(device="cpu", dtype=torch.float32)
        pred10 = self.model10(x1).to(device="cpu", dtype=torch.float32)
        #pred = (pred1 + pred2 + pred3) / 3
        pred_0 = torch.cat((pred1[:, 0:1, :, :], pred2[:, 0:1, :, :], pred3[:, 0:1, :, :], pred4[:, 0:1, :, :], pred5[:, 0:1, :, :], pred6[:, 0:1, :, :], pred7[:, 0:1, :, :], pred8[:, 0:1, :, :], pred9[:, 0:1, :, :], pred10[:, 0:1, :, :]), 1)
        pred_1 = torch.cat((pred1[:, 1:2, :, :], pred2[:, 1:2, :, :], pred3[:, 1:2, :, :], pred4[:, 1:2, :, :], pred5[:, 1:2, :, :], pred6[:, 1:2, :, :], pred7[:, 1:2, :, :], pred8[:, 1:2, :, :], pred9[:, 1:2, :, :], pred10[:, 1:2, :, :]), 1)
...
model_ft1.load_state_dict(torch.load("model/MODEL_unet_inceptionv4.pth", map_location=device0))
model_ft2.load_state_dict(torch.load("model/MODEL_unet_timm-resnest101e.pth", map_location=device0))
model_ft3.load_state_dict(torch.load("model/MODEL_manet_resnet152.pth", map_location=device0))
model_ft4.load_state_dict(torch.load("model/MODEL_unetplusplus_resnet152.pth", map_location=device0))
model_ft5.load_state_dict(torch.load("model/MODEL_unet_mobilenetv2.pth", map_location=device0))
model_ft6.load_state_dict(torch.load("model/MODEL_unet_resnet50.pth", map_location=device1))
model_ft7.load_state_dict(torch.load("model/MODEL_deeplabv3plus_mobilenetv2.pth", map_location=device1))
model_ft8.load_state_dict(torch.load("model/MODEL_unetplusplus_inceptionresnetv2.pth", map_location=device1))
model_ft9.load_state_dict(torch.load("model/MODEL_unetplusplus_resnet50.pth", map_location=device1))
model_ft10.load_state_dict(torch.load("model/MODEL_manet_mobilenetv2.pth", map_location=device1))
model_ft = EnsembledModel(model_ft1, model_ft2, model_ft3, model_ft4, model_ft5, model_ft6, model_ft7, model_ft8, model_ft9, model_ft10)
                ...
                for batch in train_loader:
                        ...
                        if division_step > 0:
                                if global_step % division_step == 0:
                                        val_score = evaluate(model_ft, val_loader)
